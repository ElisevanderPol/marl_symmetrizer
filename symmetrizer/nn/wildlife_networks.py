...
from symmetrizer.ops.wildlife_ops import get_diffs, normalize_adjacency, \
    get_message, aggregate, get_vector_state_group_representations, \
    normalize, get_wildlife_perms, get_grid_rolls
...
from symmetrizer.groups import P4, P4toInvariant, P4toActions
...
class BasisDecentralizedModel(torch.nn.Module):
    ...
    def __init__(self, input_size, n_agents, hidden_sizes=[512],
                 ...
                 paddings=[0, 0], basis="equivariant",
                 out="equivariant"):
        super().__init__()
        self.encoder = BasisWildlifeEncoder(input_size, hidden_sizes=hidden_sizes,
                                         channels=channels, filters=filters,
                                         strides=strides, paddings=paddings,
                                         gain_type='he', basis=basis,
                                         out=out)
        self.coordination = BasisWildlifeCoordinator(int(channels[-1]/np.sqrt(4)),
...
# --- Equivariant Encoder ---
class BasisWildlifeEncoder(torch.nn.Module):
    """
    """
    ...
    def forward(self, states):
        """
        """
        bs, agents, h, w = states.shape
        states = states.reshape(bs*agents, 1, h, w)
...
# --- Equivariant Coordination Network ---
class BasisWildlifeCoordinator(torch.nn.Module):
    """
    """
    ...
    def forward(self, locs, states, thresh_vec=[3, 3]):
        """
        """
...
class BasisCatGraphLayer(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, basis="equivariant",
                 gain_type="he", bias_init=False, **kwargs):
        """
        """
        super().__init__()
...
# --- Equivariant Direct Sum Group Layer ---
class SingleBasisDirectSumLayer(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size,
                 basis="equivariant",
                 gain_type="he", bias_init=False, bias=True, **kwargs):
        super().__init__()
        vector_representations = get_vector_state_group_representations()
        feature_representations = get_grid_rolls()

        in_group = direct_sum_group([feature_representations,
                                     vector_representations,
                                     feature_representations])
        out_group = get_wildlife_perms()
...
class StandardDecentralizedModel(torch.nn.Module):
    ...
    def __init__(self, input_size, n_agents, hidden_sizes=[512],
        ...
        super().__init__()
        global_pool = True
        self.encoder = StandardDecentralizedWildlifeEncoder(input_size, n_agents,
                                                         hidden_sizes=hidden_sizes,
                                                         channels=channels,
                                                         filters=filters,
                                                         strides=strides,
        ...
        self.coordinator = StandardWildlifeCoordinator(input_size, n_agents,
                                                    hidden_sizes=hidden_sizes)
...
class StandardDecentralizedWildlifeEncoder(torch.nn.Module):
    """
    """
    def __init__(self, input_size, n_agents, hidden_sizes=[512],
                 channels=[16, 32], filters=[8, 5], strides=[1, 1],
...
# --- Standard Coordination Network ---
class StandardWildlifeCoordinator(torch.nn.Module):
    """
    """
    def __init__(self, input_size, n_agents, hidden_sizes=[512],
                 filters=[8, 5], strides=[1, 1], paddings=[0, 0]):
        ...
        self.fc4 = torch.nn.Linear(input_size, 5)
...
class CatEdgeGraphLayer(torch.nn.Module):
    ...
    def forward(self, diff_vecs, agent_features, A):
        ...
        for i in range(agents):
            ...
            f_i = agent_features[:, i]
            f_next_i = aggregate([f_i, torch.cat(msgs, 1)],
                                 func=self.layer, agg_type="cat_sum")
