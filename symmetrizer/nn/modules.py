...
class BasisBiasLayer(torch.nn.Module):
    """
    Additive layer forward pass. Created to be able to separately add a bias
    term to an equivariant GCN layer.
    """
    def forward(self, x):
        """
...
class BasisAdditive(BasisBiasLayer):
    """
    Group-equivariant additive layer
    """
    def __init__(self, channels_in, channels_out, group, bias=True,
                 n_samples=4096, basis="equivariant", gain_type="xavier",
                 bias_init=False):
        ...
        if bias:
            ...
            basis_bias, self.rank_bias = get_invariant_basis(size, group,
                                                             new_size,
                                                             space=self.space)
            gain = compute_gain(gain_type, self.rank_bias, self.channels_in,
                                self.channels_out, self.repr_size_in,
                                self.repr_size_out)
    ...
    def __repr__(self):
        string = f"{self.space} Additive("
        string += f"{self.repr_size_out}, "
        string += f"{self.channels_out}), bias={self.has_bias})"
        return string
