import numpy as np
from symmetrizer.ops.wildlife_ops import get_wildlife_rolls, get_grid_rolls
from symmetrizer.ops.traffic_ops import get_traffic_rolls, \
    get_traffic_policy_perms
...
class MatrixRepresentation(Group):
    ...
    def _input_transformation(self, weights, params):
        """Input transformation comes from the input group W F_g z"""
        weights = np.matmul(weights, self._input_matrices[params])
        return weights

    def _output_transformation(self, weights, params):
        """Output transformation from the output group P_g W z"""
        weights = np.matmul(self._output_matrices[params], weights)
        return weights


class P4toActions(Group):
    """ P4 group representation, input: intermediate permutations,
    output: action permutations"""
    def __init__(self):
        ...
        self.out_permutations = get_wildlife_rolls()
...
class P4toInvariant(Group):
    """P4 group representation, input: intermediate permutations, output:
...
class P4toTraffic(Group):
    ...
    def __init__(self):
        ...
        self.repr_size_in = 4
        self.__name__ = "P4 Traffic"
        self.in_permutations = get_grid_rolls()
        self.out_permutations = get_traffic_rolls()
        self.pol_permutations = get_traffic_policy_perms()
